...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  dim f as folderitem=FindFile("image.tiff")
		  ...
		  'MsgBox str(image.columns)
		  
		  // uncomment to turn the images into a thumbnail size.
		  'image.filterType = image.LanczosFilter
		  'image.scale(new GMGeometryMBS(100,80))
		  
		  dim co as new GM16ColorRGBMBS(1.0, 0.0, 0.0)
		  ...
		  f = SpecialFolder.Desktop.Child("output16.tiff")
		  ...
		  // save 8 bit
		  blob = new GM16BlobMBS
		  
		  image.depth = 8
		  image.write blob
		  
		  f = SpecialFolder.Desktop.Child("output8.tiff")
		  ...
		  // show
		  Backdrop=image.CopyPicture(0,0,image.columns, image.rows)
