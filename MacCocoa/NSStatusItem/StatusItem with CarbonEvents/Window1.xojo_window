...
Begin Window Window1
   ...
   Visible         =   True
   Width           =   300
   Begin PushButton CreateMenuButton
      AutoDeactivate  =   True
      Bold            =   False
      ButtonStyle     =   0
      Cancel          =   False
      ...
      Width           =   112
   End
   Begin PushButton DeleteMenuButton
      AutoDeactivate  =   True
      Bold            =   False
      ButtonStyle     =   0
      Cancel          =   False
   ...
   Begin PushButton EnableButton
      ...
      Width           =   69
   End
   Begin PushButton DisableButton
      AutoDeactivate  =   True
      Bold            =   False
      ButtonStyle     =   0
      Cancel          =   False
   ...
   Begin TextField EditField1
      ...
      Width           =   155
   End
   Begin PushButton SetTitleButton
      AutoDeactivate  =   True
      Bold            =   False
      ButtonStyle     =   0
      Cancel          =   False
   ...
   Begin TextField EditField2
      ...
      Width           =   155
   End
   Begin PushButton SetTooltipButton
      AutoDeactivate  =   True
      Bold            =   False
      ButtonStyle     =   0
      Cancel          =   False
      ...
      Width           =   93
   End
   Begin PushButton AddMenuButton
      AutoDeactivate  =   True
      Bold            =   False
      ButtonStyle     =   0
      Cancel          =   False
   ...
   Begin Label StaticText1
      ...
      TabPanelIndex   =   0
      TabStop         =   True
      Text            =   "Statusitems are the small menus on the top right edge of a Mac OS X display."
      TextAlign       =   0
      TextColor       =   
      TextFont        =   "System"
      TextSize        =   0.0
      ...
      Width           =   260
   End
   Begin PushButton AddSubmenuButton
      AutoDeactivate  =   True
      Bold            =   False
      ButtonStyle     =   0
      Cancel          =   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h1
		Protected Sub ChangeImage()
		  ...
		  p.Graphics.ForeColor=rgb(rnd*127,rnd*127,rnd*127)
	...
	#tag Method, Flags = &h0
		Function MouseDown(x as integer, y as integer) As Boolean
		  ...
		  // Clicked outside of menu item?
		  If y > e.Height  + e.top or y < e.Top or x < e.left - 200 or x > e.left + e.Width then
		    return false
		  End If
		  
		  // Show menu
		  e.popUpStatusItemMenu e.Menu
	...
	#tag Property, Flags = &h1
		Protected MenuMonitorApp As monitorAppEvents
	#tag EndProperty

	#tag Property, Flags = &h1
		Protected MenuMonitorOS As monitorOSEvents
	#tag EndProperty


#tag EndWindowCode

#tag Events CreateMenuButton
	#tag Event
		Sub Action()
		  dim p,m as Picture
		  dim s as String
...
#tag Events SetTitleButton
	...
	#tag EndEvent
#tag EndEvents
#tag Events SetTooltipButton
	#tag Event
		Sub Action()
		  e.ToolTip=EditField2.text
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events AddMenuButton
	#tag Event
		Sub Action()
		  ...
		  // Send clicks missed by StatusItem to me.Mousedown
		  MenuMonitorApp = new MonitorAppEvents
		  MenuMonitorOS = new MonitorOSEvents
...
#tag Events PushButton1
	#tag Event
		Sub Action()
		  ...
		  s=s+"{\rtf1\mac\ansicpg10000\cocoartf102"+chr(13)
		  s=s+"{\fonttbl\f0\fnil\fcharset77 Zapfino;}"+chr(13)
		  s=s+"{\colortbl;\red255\green255\blue255;}"+chr(13)
		  s=s+"\paperw11900\paperh16840\margl1440\margr1440\vieww9020\viewh7500\viewkind0"+chr(13)
		  s=s+"\pard\tx566\tx1133\tx1700\tx2267\tx2834\tx3401\tx3968\tx4535\tx5102\tx5669\tx6236\tx6803\ql\qnatural"+chr(13)
		  s=s+""+chr(13)
		  s=s+"\f0\fs24 \cf0 Hello World}"
		  
		  a=new NSAttributedStringMBS
		  
		  if a.initWithRTF(s) then
		    
		    e.AttributedTitle=a
		    Timer1.Mode=0 // don't change it!
