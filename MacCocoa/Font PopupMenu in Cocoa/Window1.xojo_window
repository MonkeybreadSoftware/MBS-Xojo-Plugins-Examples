...
#tag Events PopupMenu1
	#tag Event
		Sub Open()
		  // get font list from Xojo
		  
		  dim fonts() as string
		  dim fu as integer = FontCount-1
		  for fi as integer = 0 to fu
		    fonts.append font(fi)
		  next
		  
		  fonts.Sort
		  
		  for each name as string in fonts
		    me.AddRow name
		  next
		  
		  
		  ApplyFontsToItems me
...
#tag Events PopupMenu2
	#tag Event
		Sub Open()
		  // getting font list from NSFontManager.
		  
		  dim fm as new NSFontManagerMBS
		  dim fonts() as string = fm.availableFontFamilies
