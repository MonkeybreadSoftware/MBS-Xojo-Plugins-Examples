...
Inherits AVFoundationMBS
	#tag Event
		Sub requestMediaDataWhenReadyOnQueueFinished(assetWriterInput as AVAssetWriterInputMBS, assetReaderOutput as AVAssetReaderOutputMBS, tag as variant)
		  logwindow.List.AddRow "Finished "+str(tag)
		  
		  // we do all the stuff with IDs and operations for this. To find the object here!
		  
		  dim t as TranscodeOperation = TranscodeOperation.Operation(tag)
		  t.close
