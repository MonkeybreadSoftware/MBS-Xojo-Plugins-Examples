	...
	#tag Event
		Sub Open()
		  ...
		  if TargetMacOS then
		      ...
		      path = GetFolderItem("test.db").UnixpathMBS // put the database in the current folder
		    #else
		      path = GetFolderItem("test.db").NativePath // put the database in the current folder
		    #endif
		  else
		  ...
		  if db.Connect then 
		    ...
		    // create table
		    db.SQLExecute "Create table test_tbl(fid integer, fvarchar20 varchar(20), fblob blob)"
