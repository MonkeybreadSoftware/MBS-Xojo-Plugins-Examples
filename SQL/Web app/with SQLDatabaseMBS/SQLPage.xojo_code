...
#tag Events QueryButton
	#tag Event
		Sub Pressed()
		  ...
		  dim r as RecordSet = Session.db.SQLSelect(sql)
		  
		  if Session.db.Error then
		    MsgBox Session.db.ErrorMessage
		  else
		    
		    List.ColumnCount = r.FieldCount
		    
		    dim c as integer = r.FieldCount-1
		    for x as integer = 1 to c
		      dim field as DatabaseField = r.IdxField(x)
		      List.Heading(x-1) = field.name
		    next
		    
		    while not r.EOF 
		      
		      list.AddRow r.IdxField(1).StringValue
		      
		      for x as integer = 2 to c
		        List.Cell(List.LastIndex, x) = r.IdxField(x).StringValue
		      next
