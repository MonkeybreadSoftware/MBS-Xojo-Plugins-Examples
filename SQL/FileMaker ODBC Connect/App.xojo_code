		...
		Sub Open()
		  ...
		  try
		    ...
		    elseif TargetLinux then
		      ...
		      con.SetFileOption con.kOptionLibraryODBC, findfile("odbc.so")
		    end if
		    
		    // connect to database
		    
		    // whatever connection string your ODBC server has
		    dim connectionString as string = "Driver={FileMaker ODBC};Database=Contacts;Server=127.0.0.1;PRT=2399;UID=Admin;PWD=" 
		    con.Connect(connectionString, "", "", SQLConnectionMBS.kODBCClient)
		    
		    MsgBox "We are connected!"
		    
		    dim cmd as new SQLCommandMBS(con, "SELECT * FROM Contacts")
		    cmd.Execute
		    
		    while cmd.FetchNext
		      MsgBox cmd.Field("First").asStringValue
