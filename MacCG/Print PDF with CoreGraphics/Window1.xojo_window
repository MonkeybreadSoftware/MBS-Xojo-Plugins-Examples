...
#tag Events PushButton1
	#tag Event
		Sub Action()
		  ...
		  dim pdf as CGPDFDocumentMBS = f.OpenAsCGPDFDocumentMBS
		  ...
		  for page as integer = 1 to PageCount
		    ...
		    dim r as CGRectMBS = pdf.MediaBox(page)
		    dim c as CGContextMBS
		    dim h as integer = g.Handle(g.HandleTypeCGContextRef)
		    if h <> 0 then
		      c = CGContextMBS.contextWithCGContext(h)
		    end if
		    
		    if c = nil then
		      // failed to get CGContext for printer
		      break
		    end if
		    ...
		    if r.Width > r.Height then
		      const pi = 3.14159265
		      c.RotateCTM pi*1.5+pi
		      
		      // fix origin
		      c.TranslateCTM 0, -r.Height
		    end if
		    
		    c.DrawCGPDFDocument pdf, r, page
		    
		    // cleanup
		    c.RestoreGState
		    c = nil // important, clean up!
