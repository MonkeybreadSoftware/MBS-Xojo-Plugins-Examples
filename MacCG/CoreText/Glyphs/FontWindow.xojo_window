...
#tag WindowCode
	#tag Method, Flags = &h0
		Sub run(f as CTFontMBS)
		  Title = f.FullName
		  fontFullName = f.FullName
		  fontSize = f.size
...
#tag Events List
	#tag Event
		Function CellTextPaint(g As Graphics, row As Integer, column As Integer, x as Integer, y as Integer) As Boolean
		  if column = 2 then
		    dim charCode as integer = me.RowTag(row)
		    dim text as string = encodings.UTF8.Chr(charCode)
		    
		    g.ForeColor = &c000000
		    g.TextFont = fontFullName
		    g.TextSize = fontSize
