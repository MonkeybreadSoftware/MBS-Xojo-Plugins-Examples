...
#tag WindowCode
		...
		Sub Open()
		  ...
		  if e<>Nil then
		    ...
		    while p<>Nil 
		      ...
		      // add device for this ID
		      dim name as string = p.Read(p.kDevicePropertyDevNameString)
		      dim id as string = p.Read(p.kDevicePropertyDevIdString)
		      dim d as WIAItemMBS = DeviceManager1.CreateDevice(id)
		  ...
		  if e<>Nil then
		    ...
		    while p<>Nil
		      ...
		      dim name as string = p.Read(p.kDevicePropertyDevNameString)
		      dim id as string = p.Read(p.kDevicePropertyDevIdString)
		      dim d as WIAItemMBS = DeviceManager2.CreateDevice(id)
	...
	#tag Method, Flags = &h0
		Sub AddItem(it as WIAItemMBS, name as string)
		  if it<>nil then
		    
		    if name = "" then
		      dim p as WIAPropertyStorageMBS = it.PropertyStorage
		      name = p.Read(p.kItemPropertyItemNameString)
		    end if
		    
		    // either folder, device or has attachments
		    if BitwiseAnd(it.ItemType,it.kTypeFolder + it.kTypeDevice +it.kTypeHasAttachments ) <> 0 then
		      List.addfolder name
		    else
		      List.addrow name
		    end if
