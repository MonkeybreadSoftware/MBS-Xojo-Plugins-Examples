...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  if false = WindowsWMIMBS.InitSecurity(true) then
		    MsgBox "InitSecurity failed."
		  end if
		  ...
		  // please change name, password and IP
		  if w.InitAuthentication("Christian", "", password) then
		    
		    if not w.ConnectServer("\\192.168.2.43\root\cimv2", "Christian", password) then
		...
		Sub Action()
		  ...
		  while w.NextItem
		    ...
		    if o<>Nil then
		      ...
		      List.AddRow "Path: "+o.GetPropertyString("ExecutablePath")
		    else
		      List.AddRow "o = nil"
		    end if
