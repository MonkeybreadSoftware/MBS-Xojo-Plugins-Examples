...
#tag WindowCode
	#tag Event
		Sub Open()
		  if not TargetMachO then
		    MsgBox "Please use only with MachO targets on Mac OS X!"
		    quit
		  end if
	...
	#tag Method, Flags = &h1
		Protected Function FindDevice() As MyHID
		  ...
		  if h.FindFirstDevice then
		    ...
		    if h.ProductID=&h1500 and h.VendorID=&h7C0 then
		      Return h
		    end if
	...
	#tag Method, Flags = &h0
		Sub OpenUSBDevice()
		  ...
		  if h.Lasterror<>0 then
		    MsgBox "Failed to connect to "+h.Product+" Device."
		    h=nil
		    quit
		    Return 
		  end if
	...
	#tag Method, Flags = &h0
		Sub send(n as integer)
		  ...
		  Select case h.ProductID
		  case IOWKIT_PRODUCT_ID_IOW40
		    m=NewMemoryBlock(IOWKIT40_IO_REPORT_SIZE-1)
		    m.Byte(4-1)=n
		    
		  case IOWKIT_PRODUCT_ID_IOW24
		    m=NewMemoryBlock(IOWKIT24_IO_REPORT_SIZE-1)
		    m.Byte(1-1)=n
		    
		  case IOWKIT_PRODUCT_ID_IOW56
		    m=NewMemoryBlock(IOWKIT56_IO_REPORT_SIZE-1)
		    m.Byte(7-1)=n
		  ...
		  h.SendMessageMemory(m,0,m.size)
