...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  p=new MyPhidgetMotorControlMBS
		  // -1 for first device
		  p.Open -1
	...
	#tag Property, Flags = &h1
		Protected p As PhidgetMotorControlMBS
	#tag EndProperty
...
#tag Events Timer1
	#tag Event
		Sub Action()
		  StaticText11.text=p.GetDeviceLabel
		  StaticText10.text=p.GetDeviceName
		  StaticText13.text=hex(p.GetDeviceStatus)
		  StaticText12.text=p.GetDeviceType
		  StaticText14.text=hex(p.GetDeviceVersion)
		  StaticText15.text=hex(p.GetSerialNumber)
		  
		  StaticText16.text=str(p.getMotorCount)
		  StaticText17.text=str(p.getAcceleration(0))
		  StaticText18.text=str(p.getVelocity(0))
		  StaticText20.text=str(p.getCurrent(0))
