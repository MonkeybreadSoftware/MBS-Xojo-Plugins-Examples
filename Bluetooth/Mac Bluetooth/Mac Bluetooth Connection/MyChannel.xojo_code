#tag Class
Protected Class MyChannel
Inherits IOBluetoothRFCOMMChannelMBS
	#tag Event
		Sub Closed()
		  log "Closed"
		End Sub
	...
	#tag Event
		Sub ControlSignalsChanged()
		  log "Control signals changed"
		End Sub
	#tag EndEvent

	#tag Event
		Sub DataReceived(data as MemoryBlock, dataLength as Integer)
	...
	#tag Event
		Sub FlowControlChanged()
		  log "Flow control changed"
		End Sub
	#tag EndEvent

	#tag Event
		Sub OpenCompleted(status as Integer)
		  if status = 0 then
		    log "Open completed."
		    
		  else
		    log "Open completed with error "+str(status)
		    MainWindow.MessageField.Enabled = true
	...
	#tag Event
		Sub QueueSpaceAvailable()
		  log "Queue space available."
		End Sub
	#tag EndEvent

	#tag Event
		Sub WriteCompleted(status as integer, tag as Variant)
		  log "Write completed: "+str(status)
		End Sub
	#tag EndEvent
