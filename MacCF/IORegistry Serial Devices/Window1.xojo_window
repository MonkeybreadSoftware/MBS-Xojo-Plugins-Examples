...
Begin Window Window1
   ...
   Begin Label StaticText6
      ...
      TabPanelIndex   =   0
      TabStop         =   True
      Text            =   "Here we show serial devices and their parents"
      TextAlign       =   0
      TextColor       =   
      TextFont        =   "System"
      TextSize        =   0.0
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub addnode(i as ioregistrynodeMBS)
		  if i<>nil then
		    ...
		    if i.ParentCount<=0 then
		      list.addrow i.name
		    else
		      list.addfolder i.name
		    end if
		    
		    list.cell(list.lastIndex,1)=str(i.ParentCount)
	...
	#tag Method, Flags = &h0
		Sub start()
		  dim items() as IORegistryNodeMBS = IORegistryMBS.MatchingServices("IOSerialBSDClient")
		  
		  for each item as IORegistryNodeMBS in items
		    addnode item
		  next
...
#tag Events List
	#tag Event
		Sub ExpandRow(row As Integer)
		  ...
		  dim nodes() as IORegistryNodeMBS = i.parents
