...
Begin Window Window1
   ...
   Begin Label StaticText1
      ...
      TabPanelIndex   =   0
      TabStop         =   True
      Text            =   "Select jpeg compression quality:"
      TextAlign       =   0
      TextColor       =   
      TextFont        =   "System"
      TextSize        =   0.0
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub run()
		  ...
		  je.File=f
		  je.Picture=original
		  je.Quality=Quality.Value
		  je.Export
		  
		  
		  if je.ErrorCode=0 then
		    ji.File=f
		    ji.AllowDamaged=true
		    ji.Import
		    current=ji.Picture
		    
		    size.text=str(f.length)+" Bytes"
		  else
		    msgBox "Error on creating the JPEG file. "+str(je.ErrorCode)+" "+je.ErrorMessage
		  end if
