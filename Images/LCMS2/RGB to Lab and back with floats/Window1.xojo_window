...
Begin Window Window1
   ...
   Begin PopupMenu PopupMenu1
      ...
      Index           =   -2147483648
      InitialParent   =   ""
      InitialValue    =   "INTENT_PERCEPTUAL\rINTENT_RELATIVE_COLORIMETRIC\rINTENT_SATURATION\rINTENT_ABSOLUTE_COLORIMETRIC"
      Italic          =   False
      Left            =   20
      ListIndex       =   1
      LockBottom      =   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function NewColorPicture(c as Color) As Picture
	...
	#tag Method, Flags = &h0
		Sub run(intent as integer)
		  ...
		  test canvas9,  canvas10, &c000000, StaticText6, intent
		  test canvas11, canvas12, &c777777, StaticText5, intent
	...
	#tag Method, Flags = &h0
		Sub test(canvas1 as canvas, canvas2 as canvas, c as color, info as Label, intent as integer)
		  ...
		  pi = LCMS2ProfileMBS.CreateSRGBProfile
		  
		  if pi = nil then
		    MsgBox "Failed to find the profile."+EndOfLine+EndOfLine+f.Name
		  end if
		  
		  po = LCMS2ProfileMBS.CreateLab2Profile
		  if po = nil then
		    MsgBox "Failed to find the profile."+EndOfLine+EndOfLine+f.Name
		  end if
		  
		  // get data format for 8 bit integers
		  dim pif as integer = pi.FormatterForColorspace(8, true)
		  dim pof as integer = po.FormatterForColorspace(8, true)
		  ...
		  ' ... fill c
		  mi=NewMemoryBlock(3*8) // RGB
		  mo=NewMemoryBlock(3*8) // LAB
		  
		  mi.DoubleValue( 0)=c.red / 255.0
		  mi.DoubleValue( 8)=c.green / 255.0
		  mi.DoubleValue(16)=c.blue / 255.0
		  ...
		  if ct.Transform(mi,mo,1) then
		    ...
		    // A,B = 128 is 0 point
		    // 
		    dim L as double = mo.DoubleValue( 0) // L
		    dim A as double = mo.DoubleValue( 8) // A
		    dim B as double = mo.DoubleValue(16) // B
		    
		    // convert back
		    ct = LCMS2TransformMBS.CreateTransform(po, pof, pi, pif, intent)
		    
		    if ct.Transform(mo,mi,1) then
		      dim rr as integer = mi.DoubleValue( 0)*255.0 // R
		      dim gg as integer = mi.DoubleValue( 8)*255.0 // G
		      dim bb as integer = mi.DoubleValue(16)*255.0 // B
		      
		      info.text = str(c.red)+" "+str(c.Green)+" "+str(c.Blue)+EndOfLine+_
		      str(l,"0.00")+" "+str(a,"0.00")+" "+str(b,"0.00")+EndOfLine+_
