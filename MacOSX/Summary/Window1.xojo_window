...
#tag WindowCode
	...
	#tag Method, Flags = &h1
		Protected Sub run()
		  s = new SummaryMBS(EditField1.text)
		  
		  EditField2.text = s.SentenceSummaryString(Slider1.Value)
		  
		  dim RankOrderOfSentences(-1) as integer = s.RankOrderOfSentences
		  dim SentenceIndexOfSentences(-1) as integer = s.SentenceIndexOfSentences
		  dim ParagraphIndexOfSentences(-1) as integer = s.ParagraphIndexOfSentences
		  dim RankOrderOfParagraphs(-1) as integer = s.RankOrderOfParagraphs
		  dim ParagraphIndexOfParagraphs(-1) as integer = s.ParagraphIndexOfParagraphs
		  
		  dim ParagraphCount as integer = s.ParagraphCount
		  for ParagraphIndex as integer = 0 to ParagraphCount - 1
		    popup.AddRow "Paragraph "+str(ParagraphIndex+1)+", rank "+str(RankOrderOfParagraphs(ParagraphIndex))
		    popup.RowTag(popup.ListCount-1)=ParagraphIndex
		  next
		  
		  dim SentenceCount as integer = s.SentenceCount
		  for SentenceIndex as integer = 0 to SentenceCount - 1
		    popup.AddRow "Sentence "+str(SentenceIndex+1)+", rank "+str(RankOrderOfSentences(SentenceIndex))+", paragraph "+str(ParagraphIndexOfSentences(SentenceIndex))
		    popup.RowTag(popup.ListCount-1)=SentenceIndex
		  next
	...
	#tag Event
		Sub Change()
		  ...
		  if me.ListIndex<0 then
		    EditField3.text = ""
		  elseif me.ListIndex < ParagraphCount then
		    EditField3.text = s.ParagraphAtIndex(me.ListIndex)
		  else
		    EditField3.text = s.SentenceAtIndex(me.ListIndex-ParagraphCount)
		  end if
