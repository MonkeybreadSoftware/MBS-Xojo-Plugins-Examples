...
Begin Window Window1
   ...
   Placement       =   0
   Resizeable      =   False
   Title           =   "zip folder with encryption"
   Visible         =   True
   Width           =   300
   Begin Label Label1
      AutoDeactivate  =   True
      ...
      TabPanelIndex   =   0
      TabStop         =   True
      Text            =   "Done. Check files on desktop."
      TextAlign       =   0
      TextColor       =   
      TextFont        =   "System"
      TextSize        =   0.0
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub ZipFile(password as string, z as ZipMBS, f as FolderItem, path as string)
		  const WindowBits = 15
		  const memLevel = 8
		  ...
		  z.CreateFile path+f.name, info, "", "", "", z.MethodDeflated, z.CompressionBestCompression, false, false, WindowBits, memLevel, z.StrategyDefault, password, crc
		  z.Write data
		  z.CloseFile
	...
	#tag Method, Flags = &h0
		Sub ZipFolder(password as string, z as ZipMBS, source as FolderItem, path as string)
		  dim f as FolderItem
		  dim i,c as integer
		  ...
		  for i=1 to c
		    ...
		    elseif f.Directory then
		      ZipFolder password, z, f, path
		    else
		      ZipFile password, z, f, path
		    end if
		  next
