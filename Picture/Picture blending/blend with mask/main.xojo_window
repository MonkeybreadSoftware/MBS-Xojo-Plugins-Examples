...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub img_proc(bal as integer)
		  ...
		  m = new Picture(img_a.Width,img_a.Height,32)
		  m.Graphics.ForeColor=Rgb(bal,bal,bal)
		  m.Graphics.FillOval 0,0,m.Width,m.Height
		  m.Graphics.ForeColor=Rgb(0,0,0)
		  m.Graphics.FillOval 40,40,m.Width-80,m.Height-80
		  
		  t=ticks
		  for i=1 to 100
		    img_res= BlendPicturesWithMaskMBS(img_a,img_b,m)
		  next
		  Title=str(ticks-t)
