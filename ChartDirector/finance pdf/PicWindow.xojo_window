...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  Dim dpi As Integer = 96 // default for ChartDirector
		  
		  // we use DPI to scale down graphics
		  Dim f As Double = (c.getWidth / pagewidth)
		  dpi        = f * dpi
		  pagewidth  = f * pagewidth
		  pageheight = f * pageheight
		  
		  Call c.setOutputOptions("dpi="+Str(dpi)+";pagewidth="+Str(pagewidth)+";pageheight="+Str(pageheight))
		  
		  Dim file As FolderItem = SpecialFolder.Desktop.Child("finance.pdf")
