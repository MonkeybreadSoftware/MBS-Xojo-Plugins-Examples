...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  for i as integer = 0 to 86400-1 step 60 // one minute
		    ...
		    lastdatas.Append CDBaseChartMBS.kNoValue
		    times.Append i
	...
	#tag Method, Flags = &h0
		Sub redraw()
		  ...
		  dim l2 as CDLineLayerMBS = c.addLineLayer(lastdatas, &c777777)
		  l2.setXData(times)
...
#tag Events Timer1
	#tag Event
		Sub Action()
		  ...
		  LastValue = datas(time)
		  // add no value to break line
		  datas(time) = CDBaseChartMBS.kNoValue
		  LastDatas(time) = CDBaseChartMBS.kNoValue
