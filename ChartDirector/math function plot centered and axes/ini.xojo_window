...
#tag Events out
	#tag Event
		Function Render(ViewPort As CDViewPortManagerMBS, Factor As Double, Width As Integer, Height As Integer) As CDBaseChartMBS
		  'dim xminValue  as Double = val(xmin.Text)
		  'dim xmaxValue  as Double = val(xmax.Text)
		  dim xminAxis  as Double = val(Axmin.Text)
		  dim xmaxAxis  as Double = val(Axmax.Text)
		  dim yminAxis  as Double = val(Aymin.Text)
		  dim ymaxAxis  as Double = val(Aymax.Text)
		  ...
		  dim x as integer = 75*factor
		  dim y as integer = 55*factor
		  dim w as integer = (width-150)*factor
		  dim h as integer = (height-150)*factor
		  
		  call c.setPlotArea(x, y, w, h, c.linearGradientColor(0, 55, 0, 335, &hf9fcff, &haaccff), -1, c.kTransparent, &hffffff).setGridColor(&hFF0000, &hFF0000)
		  call c.setClipping
