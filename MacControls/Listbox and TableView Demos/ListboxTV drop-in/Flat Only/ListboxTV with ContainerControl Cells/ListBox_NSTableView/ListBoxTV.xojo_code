...
Inherits NSTableControlMBS
	#tag Event
		Function acceptDrop(info as NSDraggingInfoMBS, row as Integer, dropOperation as Integer) As Boolean
		  // Finishes the drop
	...
	#tag Event
		Function dataCell(tableColumn as NSTableColumnMBS, row as Int64) As NSCellMBS
	...
	#tag Event
		Function shouldEditTableColumn(tableColumn as NSTableColumnMBS, row as Int64) As Boolean
	...
	#tag Event
		Function shouldReorderColumn(columnIndex as Int64, newColumnIndex as Int64) As Boolean
		  return not RaiseEvent DenyReorderColumn (columnIndex, newColumnIndex)
		End Function
	#tag EndEvent

	#tag Event
		Function validateDrop(info as NSDraggingInfoMBS, proposedRow as Integer, dropOperation as Integer) As Integer
	...
	#tag Event
		Sub willDisplayCell(cell as NSCellMBS, tableColumn as NSTableColumnMBS, row as Int64)
		  #if Support_OSX_10_6
	...
	#tag Event
		Function writeRowsWithIndexes(rowIndexes as NSIndexSetMBS, pboard as NSPasteboardMBS) As Boolean
	...
	#tag Note, Name = About
		...
		font of the cells and text. See the note "Handling CellBackgroundPaint and CellTextPaint" for a possible solution.
	...
	#tag Note, Name = On-demand Cell values (custom DataSource)
		...
		• DataSource_RowCount() as Integer
		...
		and DataSource_Cell() functions again, call the listbox's Reload() function.
	...
	#tag Note, Name = To Do
		...
		• Hierarchical (use the code from "ListBoxTV OutlineView.rbp" instead)
		• Row Insert + Delete ("ListBoxTV OutlineView.rbp" has a RemoveRow method, at least)
	...
	#tag Property, Flags = &h21
		Private mTableView As NSTableViewMBS
	#tag EndProperty
