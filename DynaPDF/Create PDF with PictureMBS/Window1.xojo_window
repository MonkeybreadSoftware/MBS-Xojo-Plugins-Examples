...
#tag WindowCode
	#tag Event
		Sub Open()
		  // This example shows how to embed a Picture in a PictureMBS into a PDF
		  
		  // if you have a Picture object, please use InsertPicture command.
		  ...
		  file=SpecialFolder.Desktop.Child("Create PDF with PictureMBS.pdf")
		  ...
		  // get picture
		  dim p as PictureMBS=MakePicture
		  ...
		  // add image to pdf
		  dim m as MemoryBlock = p.MemoryTarget
		  call d.InsertRawImage(m, 32, 0, p.Width, p.Height, x,y,w,h)
	...
	#tag Method, Flags = &h0
		Function MakePicture() As PictureMBS
		  dim p as Picture
		  ...
		  dim m as MemoryBlock = NewMemoryBlock(2062*2062*4)
		  dim x as new PictureMBS(m, 2062,2062, PictureMBS.ImageFormatBGRX, 2062*4)
		  
		  // draw RB Picture into a mirrored view for DynaPDF.
		  x.MirroredView.DrawPictureRGB(p, 0, 0, p.Width, p.Height)
		  
		  Backdrop=p.ScaleMBS(Width,Height)
