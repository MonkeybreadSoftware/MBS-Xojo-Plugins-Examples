...
Begin Window Window1
   ...
   Begin TextArea TextArea1
      ...
      TabPanelIndex   =   0
      TabStop         =   True
      Text            =   "You can keep this window on top above other windows if you like."
      TextColor       =   
      TextFont        =   "System"
      TextSize        =   0.0
      TextUnit        =   0
...
#tag Events CheckBox1
	#tag Event
		Sub Action()
		    ...
		    static NormalWindowGroup as WindowGroupMBS
		    static FloatWindowGroup as WindowGroupMBS
		  ...
		  if me.Value then
		      ...
		      w.Level = w.NSStatusWindowLevel
		    #Elseif TargetCarbon then
		      // Mac OS X Carbon
		      
		      // make copy of normal group for later
		      if NormalWindowGroup = nil then
		        NormalWindowGroup = self.WindowGroupMBS
		      end if
		      
		      // now build a custom group for floating
		      ...
		      if FloatWindowGroup = nil then
		        FloatWindowGroup = new WindowGroupMBS
		        FloatWindowGroup.Create(0)
		        FloatWindowGroup.Level = FloatWindowGroup.LevelForKey(kCGStatusWindowLevelKey)
		        
		      end if
		      
		      self.WindowGroupMBS = FloatWindowGroup
		      
		    #Elseif TargetLinux then
		      dim g as new GTKWindowMBS(self)
		      g.SetKeepAbove(true)
		    #else
		      ?
		    #endif
		    ...
		    #if TargetWin32 then
		      self.WinTopMostWindowMBS = false
		    #elseif TargetCocoa then
		      dim w as NSWindowMBS = self.NSWindowMBS
		      w.Level = w.NSNormalWindowLevel
		    #Elseif TargetCarbon then
		      
		      self.WindowGroupMBS = NormalWindowGroup
		      
		    #Elseif TargetLinux then
		      dim g as new GTKWindowMBS(self)
		      g.SetKeepAbove(false)
		    #else
		      ?
		    #endif
